-- =====================================================================
--  FreshServer.sql   -   FateMMO / FateRO  bootstrap schema
-- =====================================================================
--  ONE-COMMAND FRESH INSTALL (main game schema):
--
--      mysql -u root -p ragnarok_main < FreshServer.sql
--
--  This file = stock rAthena `main` schema  +  every DB customization
--  this server needs.  It does NOT contain player/runtime data and is
--  safe to run once on a COMPLETELY EMPTY `ragnarok_main`.
--
--  Base rAthena commit:  673c3c8  (fork of rAthena, sql-files/main.sql)
--  Generated per FreshServer_Audit.md   -   do not hand-edit; regenerate.
--
--  SEPARATE SCHEMAS (not bundled here - run once each, see audit):
--      mysql -u root -p ragnarok_logs < sql-files/logs.sql
--      mysql -u root -p ragnarok_web  < sql-files/web.sql
--      # plus custom log table `promotion_log` - see appendix at EOF
--
--  Layout:
--    SECTION 1  base rAthena schema           (sql-files/main.sql)
--    SECTION 2  standard data                 (roulette)
--    SECTION 3  custom tables
--    SECTION 4  custom alterations (indexes)
--    SECTION 5  custom seed / config data
--    SECTION 6  optional / ambiguous (commented)
--    APPENDIX   separate-schema custom bits (commented)
-- =====================================================================
...
-- =====================================================================
--  SECTION 1 : BASE rATHENA SCHEMA        source: sql-files/main.sql
-- =====================================================================
--
...
-- =====================================================================
--  SECTION 2 : STANDARD DATA             source: sql-files/roulette_default_data.sql
-- =====================================================================
-- ----------------------------
...
-- ---------------------------------------------------------------------
--  Card / Armor Deposit           source: deposit.sql  (repo root)
--  Used by: npc/custom/card_deposit.txt, npc/custom/Vending.txt
-- ---------------------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS `armordeposit` (
...
-- ---------------------------------------------------------------------
--  AutoBuff / Auto Support        source: sql-files/AutoBuff.sql
--  Used by: npc/custom/autobuff.txt  + src/map/autobuff.cpp
--  (InnoDB - foreign keys; create parent tables first)
-- ---------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS `ab_common_config` (
...
-- Composite UNIQUE keys are declared inline (not as trailing ALTERs) so
-- MariaDB reuses them as the FK supporting index instead of auto-creating
-- a second `char_id` index - this reproduces the live schema exactly.
CREATE TABLE IF NOT EXISTS `ab_skills` (
    ...
    UNIQUE KEY `char_id` (`char_id`,`type`,`skill_id`,`skill_lv`),
    FOREIGN KEY (`char_id`) REFERENCES `ab_common_config`(`char_id`)
) ENGINE=InnoDB;

CREATE TABLE IF NOT EXISTS `ab_skills_char_ids` (
    `ab_skills_id` BIGINT UNSIGNED NOT NULL,
    `ab_char_id` INT UNSIGNED NOT NULL,
    UNIQUE KEY `chars_id` (`ab_skills_id`,`ab_char_id`),
...
CREATE TABLE IF NOT EXISTS `ab_pm_char_ids` (
    `char_id` INT UNSIGNED NOT NULL,
    `ab_char_id` INT UNSIGNED NOT NULL,
    UNIQUE KEY `char_ab_unique` (`char_id`,`ab_char_id`),
    FOREIGN KEY (`char_id`) REFERENCES `ab_common_config`(`char_id`)
) ENGINE=InnoDB;

-- ---------------------------------------------------------------------
--  Emoticon packs                 source: sql-files/Emoticon.sql
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
--  Population Engine stats        source: sql-files/population_engine.sql
--  (also auto-created by src/map/population_engine.cpp on boot)
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
--  Donor Rewards Redeemer log     source: sql-files/cp_redeemlog.sql
--  Used by: npc/custom/DonateRedeem.txt (+ FluxCP donation approvals)
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
--  PvP ladder                     source: npc/custom/ladder_pvp.txt
-- ---------------------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS `ownpvpladder` (
...
-- ---------------------------------------------------------------------
--  WoE ladder                     source: npc/custom/ladder_woe.txt
-- ---------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS `woeladder` (
	`char_id` INT PRIMARY KEY,
	`name` VARCHAR(24) NOT NULL DEFAULT '',
	`streaks` INT DEFAULT 0,
	`kills` INT DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS `ownwoeladder` (
	`guild_id` INT PRIMARY KEY,
	`name` VARCHAR(24) NOT NULL DEFAULT '',
	`currentown` INT DEFAULT 0,
	`highestown` INT DEFAULT 0,
...
-- ---------------------------------------------------------------------
--  WoE Emperium Breaker ranking   source: npc/custom/ladder_woe.txt
--  (OnNPCKillEvent on Emperium mob 1288 during agitcheck / agitcheck2)
-- ---------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS `woe_breaker` (
	`char_id` INT PRIMARY KEY,
	`name` VARCHAR(24) NOT NULL DEFAULT '',
...
CREATE TABLE IF NOT EXISTS `woe_breaker_guild` (
	`guild_id` INT PRIMARY KEY,
	`name` VARCHAR(24) NOT NULL DEFAULT '',
	`breaks` INT NOT NULL DEFAULT 0,
	`last_break` DATETIME,
...
-- ---------------------------------------------------------------------
--  MVP ladder                     source: npc/custom/ladder_mvp.txt
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
--  Brewer ranking                 source: npc/custom/Brewer.txt
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
--  Daily / Rank mission points    source: npc/custom/DailyQuest.txt
-- ---------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS `rank_mission` (
  `char_id` int(11) NOT NULL,
...
-- ---------------------------------------------------------------------
--  Promotion codes                source: npc/custom/PromoCode.txt
--  (promotion_log lives in the LOG schema - see section 0)
-- ---------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS `promotion_codes` (
  ...
  `protection` ENUM('0','1','2','3') NOT NULL default '0',
  PRIMARY KEY  (`id`)
) ENGINE=MyISAM;
...
CREATE TABLE IF NOT EXISTS `promotion_history` (
  ...
  `protection_id` INT(11) UNSIGNED NOT NULL default '0',
...
-- ---------------------------------------------------------------------
--  Auto Combat                    source: src/map/autocombat.cpp
--  (map-server also auto-creates these on boot; here for completeness)
-- ---------------------------------------------------------------------
...
-- =====================================================================
--  SECTION 4 : CUSTOM ALTERATIONS  (performance indexes)
--  source: sql-files/upgrades/upgrade_20260907.sql
--  Purely additive - speeds up item-ownership lookups (Searcher NPC,
--  web panels) and the MusicBox acc_reg_num cleanup. MariaDB 10.5+/MySQL 8+.
-- =====================================================================
...
-- Extended Vending system [Lilith / Easycore]  (src/map/vending.cpp)
-- Stores the currency item id a vend shop uses (0 = plain zeny) so autotrade
...
-- ---------------------------------------------------------------------
--  Monster Arena - npc/custom/MonsterArena.txt is currently DISABLED in
--  npc/scripts_custom.conf. Its tables exist in the live DB from earlier
--  testing. Uncomment only if you re-enable that script.
-- ---------------------------------------------------------------------
...
-- ) ENGINE=MyISAM DEFAULT CHARSET=utf8;
-- CREATE TABLE IF NOT EXISTS `arena_drop_settings` (
--   `id` int(11) NOT NULL AUTO_INCREMENT,
--   `setting_type` varchar(20) NOT NULL,
--   `setting_key` varchar(50) NOT NULL,
--   `setting_value` int(11) NOT NULL,
--   `extra_value` int(11) DEFAULT NULL,
--   `created_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
--   PRIMARY KEY (`id`),
--   UNIQUE KEY `unique_setting` (`setting_type`,`setting_key`)
-- ) ENGINE=MyISAM DEFAULT CHARSET=utf8;
-- ---------------------------------------------------------------------
--  NOT included (see FreshServer_Audit.md):
...
--       mysql ragnarok_main < sql-files/item_db_re_equip.sql   (etc.)
-- =====================================================================

-- =====================================================================
--  APPENDIX : CUSTOM BITS THAT LIVE IN OTHER SCHEMAS (commented)
-- ---------------------------------------------------------------------
--  promotion_log  ->  LOG schema (conf/inter_athena.conf::log_db_db,
--  default `ragnarok_logs`). Written via query_logsql() from
--  npc/custom/PromoCode.txt. Run against the log schema:
--
--    USE `ragnarok_logs`;
