...
{
	return session[fd]->fateshield_info.unique_id;
}
...
void fateshield_session_unit(unsigned int seed, struct fateshield_crypt_unit* unit)
{
	unsigned int i;
...
bool is_fateshield_active = false;
unsigned int fateshield_rand_seed;
unsigned int min_allowed_license_version;

inline void fateshield_srand(unsigned int seed)
{
	fateshield_rand_seed = seed;
}

inline unsigned int fateshield_rand(void)
{
	return (((fateshield_rand_seed = fateshield_rand_seed * 214013L + 2531011L) >> 16) & 0x7fff);
}

unsigned int fateshield_get_data_hash(const unsigned char* data, unsigned int data_size)
{
...
void fateshield_read_configs(void)
{
	FILE* fp;
	char line[1024], w1[1024], w2[1024];
	const char* conf_name = "conf/fateshield.conf";

	start_tick = fateshield_get_tick();
	...
	if (access(conf_name, 0) == -1)
	{
		...
		if (fp == NULL) 
		{
			ShowError("FateShield can't create configuration file: %s\n", conf_name);
			exit(EXIT_FAILURE);
		}

		fprintf(fp, "fateshield_enabled: yes");
		fclose(fp);

		is_fateshield_active = true;
	...
	if (fp == NULL) 
	{
		ShowError("FateShield can't open configuration file: %s\n", conf_name);
		exit(EXIT_FAILURE);
	}
	...
	while (fgets(line, sizeof(line), fp))
	{
		...
		if (!strcmpi(w1, "fateshield_enabled"))
		{
			is_fateshield_active = (unsigned int)config_switch(w2);
		}
	}
...
void fateshield_init(struct socket_data* s, int fd, unsigned short server_type)
{
	...
	fateshield_session_unit(recv_session_key, &s->recv_crypt);
	fateshield_session_unit(send_session_key, &s->send_crypt);
	fateshield_session_unit(sync_session_key, &s->sync_crypt);

	WFIFOHEAD(fd, init_packet_size);

	WFIFOW(fd, 0) = SC_FATESHIELD_INIT;
	...
	WFIFOL(fd, 10) = fateshield_get_data_hash(WFIFOP(fd, 14), init_packet_size - 14);
...
bool fateshield_process_cs_packet(int fd, struct socket_data* s, size_t packet_size)
{
	...
	if (packet_id == s->fateshield_info.cs_packet_request_move ||
		packet_id == s->fateshield_info.cs_packet_action_request ||
		packet_id == s->fateshield_info.cs_packet_use_skill_to_id ||
		packet_id == s->fateshield_info.cs_packet_use_skill_to_ground)
	{
		...
		fateshield_enc_dec(packet_data + 2, packet_size - 2, &s->recv_crypt);
	...
	switch (packet_id)
	{
		case CS_FATESHIELD_SYNC:
		{
			...
			fateshield_enc_dec(packet_data + 2, 4, &s->sync_crypt);
		...
		case CS_LOGIN_PACKET_7:
		{
			...
			if (s->fateshield_info.is_init_ack_received == false)
			{
				RFIFOSKIP(fd, RFIFOREST(fd));
				fateshield_init(s, fd, FATESHIELD_LOGIN);
				return true;
			}

			fateshield_enc_dec(packet_data + 4, RFIFOREST(fd) - 4, &s->recv_crypt);
		}
		break;
		...
		case CS_WHISPER_TO:
		{
			...
			fateshield_enc_dec(packet_data + 4, packet_size - 4, &s->recv_crypt);
		}
		break;

		case CS_FATESHIELD_INIT_ACK:
		{
			...
			if (packet_size < 44)
			{
				ShowWarning("fateshield_process_cs_packet: invalid size of CS_FATESHIELD_INIT_ACK packet: %lu\n", (unsigned long)packet_size);
				fateshield_set_eof(fd);
				return true;
			}
			...
			data_hash_2 = fateshield_get_data_hash(packet_data + 8, packet_size - 8);

			if (data_hash_1 != data_hash_2)
			{
				fateshield_send_info(fd, FATESHIELD_INFO_INVALID_INIT_ACK, NULL);
				return true;
			}

			s->fateshield_info.unique_id = RFIFOL(fd, 16);
			s->fateshield_info.license_version = RFIFOL(fd, 12);
			s->fateshield_info.cs_packet_request_move = RFIFOW(fd, 28);
			s->fateshield_info.cs_packet_use_skill_to_id = RFIFOW(fd, 30);
			s->fateshield_info.cs_packet_use_skill_to_ground = RFIFOW(fd, 32);
			s->fateshield_info.cs_packet_action_request = RFIFOW(fd, 34);
			memcpy(s->fateshield_info.mac_address, RFIFOP(fd,20), sizeof(s->fateshield_info.mac_address));

			if (packet_size >= 48)
			{
				unsigned int real_ip = RFIFOL(fd, 44);

				if (real_ip)
					s->client_addr = real_ip;
			}

			s->fateshield_info.is_init_ack_received = true;
...
void fateshield_process_sc_packet(int fd, struct socket_data* s, size_t packet_size)
{
	...
	switch (packet_id)
	{
		case SC_FATESHIELD_INIT:
		{
			fateshield_enc_dec(packet_data + 10, packet_size - 10, &s->send_crypt);
		}
		break;
		...
		{
			fateshield_enc_dec(packet_data + 4, packet_size - 4, &s->send_crypt);
		}
		break;

		case SC_STATE_CHANGE:
		{
			fateshield_enc_dec(packet_data + 6, 8, &s->send_crypt);
		}
		break;
		...
		{
			fateshield_enc_dec(packet_data + 2, 4, &s->send_crypt);
		}
		break;
	}
}
...
void fateshield_send_info(int fd, unsigned short info_type, const char* message)
{
	...
	WFIFOW(fd, 0) = SC_FATESHIELD_INFO;
...
// (^~_~^) FateShield Start
	...
	if (is_fateshield_active == true && global_core->get_type() != e_core_type::CHARACTER)
	{
		fateshield_process_sc_packet(fd, s, len);
	}
