...
int chmapif_parse_fateshield_save_report(int fd)
{
	unsigned int offset;
	...
	char report_str_esc[FATESHIELD_REPORT_LENGTH];

	unsigned int packet_len = (2 + 4 + 4 + 4 + NAME_LENGTH + FATESHIELD_REPORT_LENGTH);
	...
	Sql_EscapeStringLen(sql_handle, report_str_esc, report_str, strnlen(report_str, FATESHIELD_REPORT_LENGTH));

	RFIFOSKIP(fd, packet_len);

	if (SQL_ERROR == Sql_Query(sql_handle, "INSERT INTO `fateshield_report_log` (`time`, `unique_id`,`account_id`,`char_id`,`char_name`,`report_str`)"
...
int chmapif_parse_fateshield_block(int fd)
{
	unsigned int unique_id;
	char* sql_data;
	char result_str[FATESHIELD_RESULT_STR_LENGTH];
	char reason_str_esc[FATESHIELD_REASON_LENGTH*2+1];
	char unban_time_str_esc[FATESHIELD_TIME_STR_LENGTH*2+1];
	...
	unsigned int packet_len = (2 + 4 + 4 + 4 + NAME_LENGTH + NAME_LENGTH + FATESHIELD_TIME_STR_LENGTH + FATESHIELD_REASON_LENGTH);

	safesnprintf(result_str, FATESHIELD_RESULT_STR_LENGTH, "Result: unkqnown");
	...
	offset += FATESHIELD_TIME_STR_LENGTH;

	reason_str = (const char*)RFIFOP(fd,offset);
	offset += FATESHIELD_REASON_LENGTH;
	...
	while ("FateShield")
	{
		...
		if (violator_aid == 0 && *violator_name != '\0')
		{
			...
			{
				Sql_ShowDebug(sql_handle);
				safesnprintf(result_str, FATESHIELD_RESULT_STR_LENGTH, "Result: An error has been occurred. You should see console..");
				break;
			}
			else if (Sql_NumRows(sql_handle) == 0)
			{
				safesnprintf(result_str, FATESHIELD_RESULT_STR_LENGTH, "Result: The player has not found.");
				break;
			}
		...
		if (unique_id == 0)
		{
			if (violator_aid == 0)
			{
				safesnprintf(result_str, FATESHIELD_RESULT_STR_LENGTH, "Result: unique_id and violator_aid == 0! ERROR");
				break;
			}
			...
			{
				safesnprintf(result_str, FATESHIELD_RESULT_STR_LENGTH, "Result: The account has not been found.");
				break;
			}
			else if (SQL_SUCCESS != Sql_NextRow(sql_handle))
			{
		...
		if (unique_id == 0)
		{
			safesnprintf(result_str, FATESHIELD_RESULT_STR_LENGTH, "Result: You can not block unique_id which equal 0.");
			break;
		}
		...
		if (SQL_ERROR == Sql_Query(sql_handle, "SELECT * FROM `fateshield_block` WHERE `unique_id` = '%u'", unique_id))
		{
			safesnprintf(result_str, FATESHIELD_RESULT_STR_LENGTH, "Result: An error has been occurred. You should see console..");
			Sql_ShowDebug(sql_handle);
			break;
		...
		{
			if (SQL_ERROR == Sql_Query(sql_handle, "DELETE FROM `fateshield_block` WHERE `unique_id` = '%u'", unique_id))
			{
				Sql_ShowDebug(sql_handle);
			}
		}

		Sql_EscapeStringLen(sql_handle, unban_time_str_esc, unban_time_str, strnlen(unban_time_str, FATESHIELD_TIME_STR_LENGTH));
		Sql_EscapeStringLen(sql_handle, initiator_name_esc, initiator_name, strnlen(initiator_name, NAME_LENGTH));
		Sql_EscapeStringLen(sql_handle, reason_str_esc, reason_str, strnlen(reason_str, FATESHIELD_REASON_LENGTH));

		if (SQL_ERROR == Sql_Query(sql_handle, "INSERT INTO `fateshield_block` (`unique_id`, `unban_time`, `reason`) VALUES ('%u', '%s', '%s')",
		...
		if (SQL_ERROR == Sql_Query(sql_handle, "INSERT INTO `fateshield_block_log` (`unique_id`,`block_time`,`unban_time`,`violator_name`,`violator_account_id`,`initiator_name`,`initiator_account_id`,`reason`)"
		...
		safesnprintf(result_str, FATESHIELD_RESULT_STR_LENGTH, "Result: Success!");
	...
	WFIFOHEAD(fd, 2 + 4 + 4 + 4 + FATESHIELD_TIME_STR_LENGTH + FATESHIELD_TIME_STR_LENGTH + FATESHIELD_RESULT_STR_LENGTH);
	WFIFOW(fd, 0) = FATESHIELD_C2M_BLOCK_ACK;
	WFIFOL(fd, 2) = unique_id;
	WFIFOL(fd, 6) = violator_aid;
	WFIFOL(fd,10) = initiator_aid;
	offset = (2 + 4 + 4 + 4);

	safestrncpy((char*)WFIFOP(fd, offset), (const char*)RFIFOP(fd, offset), FATESHIELD_TIME_STR_LENGTH);
	offset += FATESHIELD_TIME_STR_LENGTH;

	safestrncpy((char*)WFIFOP(fd, offset), (const char*)RFIFOP(fd, offset), FATESHIELD_REASON_LENGTH);
	offset += FATESHIELD_REASON_LENGTH;

	safestrncpy((char*)WFIFOP(fd, offset), result_str, FATESHIELD_RESULT_STR_LENGTH);
	offset += FATESHIELD_RESULT_STR_LENGTH;
...
int chmapif_parse_fateshield_unblock(int fd)
{
	unsigned int unique_id;
	char* sql_data;
	char result_str[FATESHIELD_RESULT_STR_LENGTH];
	...
	while ("FateShield")
	{
		...
		if (violator_aid != 0)
		{
			...
			if (unique_id == 0)
			{
				safesnprintf(result_str, FATESHIELD_RESULT_STR_LENGTH, "Result: account don't have block information.");
				break;
			}
		}
	...
	WFIFOHEAD(fd, 2 + 4 + FATESHIELD_RESULT_STR_LENGTH);
	WFIFOW(fd, 0) = FATESHIELD_C2M_UNBLOCK_ACK;
...
// (^~_~^) FateShield Start
			...
			case FATESHIELD_M2C_BLOCK_REQ: next=chmapif_parse_fateshield_block(fd); break;
			case FATESHIELD_M2C_UNBLOCK_REQ: next=chmapif_parse_fateshield_unblock(fd); break;
			case FATESHIELD_M2C_SAVE_REPORT: next = chmapif_parse_fateshield_save_report(fd); break;
