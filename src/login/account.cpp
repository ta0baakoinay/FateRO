...
void account_fateshield_init(AccountDB* self)
{
...
void account_fateshield_update_last_unique_id(int account_id, unsigned int unique_id)
{
	if (is_fateshield_active == false)
	{
		return;
	}
...
bool account_fateshield_check_unique_id(int fd, struct socket_data* s)
{
	unsigned int unique_id = s->fateshield_info.unique_id;

	if (SQL_SUCCESS != Sql_Query(db_sql->accounts, "SELECT `unban_time`, `reason` FROM `fateshield_block` WHERE `unique_id` = '%u'", unique_id))
	{
		Sql_ShowDebug(db_sql->accounts);
		fateshield_send_info(fd, FATESHIELD_INFO_MESSAGE, "Tell administrator about SQL problem.");
	}
	else if (SQL_SUCCESS == Sql_NextRow(db_sql->accounts))
	{
		char* data;
		...
		if (time_server <= time_unban)
		{
			...
			s->fateshield_info.is_init_ack_received = false;

			fateshield_send_info(fd, FATESHIELD_INFO_BANNED, message_info);
		}
		else if (SQL_ERROR == Sql_Query(db_sql->accounts, "DELETE FROM `fateshield_block` WHERE `unique_id` = '%u'", unique_id))
		{
			Sql_ShowDebug(db_sql->accounts);
		}
	}
