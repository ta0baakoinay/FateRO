...
struct s_population {
	...
	// --- Ambient chat ---
	uint32_t last_chat_hash = 0;  ///< FNV-1a hash of this shell's last spoken line; the picker re-rolls once if it would repeat it.
	uint8_t  chattiness     = 50; ///< 0-100 personality weight: per eligible chat tick the shell also has to pass rnd()%100 < chattiness. Low = quiet, high = talkative. Set at spawn.

	// --- Population ownership ---
	uint8_t  source = 0;         ///< 0 = manual (GM @populate, never auto-replenished), 1 = auto (belongs to an @populate auto target; the auto manager keeps its map topped up).
	...
	// --- Stuck detection / recovery (wander timer) ---
	int16_t stuck_x         = -1; ///< Position at the previous stuck-check.
	int16_t stuck_y         = -1;
	t_tick  stuck_since     = 0;  ///< First tick the position was seen unchanged (0 = not tracking).
	t_tick  last_reloc_tick = 0;  ///< Tick of the last forced relocation (recovery cooldown).
	uint8_t reloc_count     = 0;  ///< Consecutive forced relocations without a clean move in between (capped).

	// --- Town personality phase cycler (Sit / IdleStand <-> ShortWalk) ---
	t_tick  town_phase_until = 0; ///< Tick the current town behaviour phase ends (0 = not a cycled town shell).
	...
	// --- Proactive floor-loot scan ---
	t_tick next_loot_scan   = 0; ///< Next tick this shell may scan for nearby floor drops.
	uint32 loot_walk_target = 0; ///< flooritem id the shell is currently walking to pick up (0 = none).
