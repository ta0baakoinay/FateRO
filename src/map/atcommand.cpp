...
ACMD_FUNC(fateshield_block_nick)
{
	map_session_data* violator_sd;
	time_t time_server;
	unsigned int duration;
	unsigned int violator_account_id = 0;
	unsigned int violator_unique_id = 0;
	char reason_str[FATESHIELD_REASON_LENGTH];
	char unban_time_str[FATESHIELD_TIME_STR_LENGTH];
	char duration_type, violator_name[NAME_LENGTH];
	const char* command_info = "Wrong input (usage: @fateshield_block_nick <duration> <duration_type m/h/d> \"<char name>\" <reason>)";
	...
	{
		case 'm': time_server += (duration * 60); break;
		case 'h': time_server += (duration * 3600); break;
	...
	if (violator_sd != NULL)
	{
		...
		violator_unique_id = session[violator_sd->fd]->fateshield_info.unique_id;
	}

	chrif_fateshield_req_block(violator_unique_id, violator_name, violator_account_id, sd->status.name, sd->status.account_id, unban_time_str, reason_str);

	return 0;
}

ACMD_FUNC(fateshield_block_account_id)
{
	map_session_data* violator_sd;
	time_t time_server;
	unsigned int duration;
	...
	unsigned int violator_unique_id = 0;
	char reason_str[FATESHIELD_REASON_LENGTH];
	char duration_type, unban_time_str[FATESHIELD_TIME_STR_LENGTH];
	const char* command_info = "Wrong input (usage: @fateshield_block_account_id <duration> <duration_type m/h/d> <account ID> <reason>)";
	...
	chrif_fateshield_req_block(violator_unique_id, atcmd_player_name, violator_account_id, sd->status.name, sd->status.account_id, unban_time_str, reason_str);

	return 0;
}

ACMD_FUNC(fateshield_block_unique_id)
{
	time_t time_server;
	unsigned int duration;
	unsigned int violator_unique_id = 0;
	char reason_str[FATESHIELD_REASON_LENGTH];
	char duration_type, unban_time_str[FATESHIELD_TIME_STR_LENGTH];
	const char* command_info = "Wrong input (usage: @fateshield_block_unique_id <duration> <duration_type m/h/d> <unique ID> <reason>)";
	...
	sprintf(atcmd_output, "Request: block by unique ID: %u", violator_unique_id);
	clif_displaymessage(fd, atcmd_output);

	chrif_fateshield_req_block(violator_unique_id, NULL, 0, sd->status.name, sd->status.account_id, unban_time_str, reason_str);

	return 0;
}

ACMD_FUNC(fateshield_unblock_nick)
{
	char violator_name[NAME_LENGTH];
	const char* command_info = "Wrong input (usage: @fateshield_unblock_nick <char name>)";
	...
	chrif_fateshield_req_unblock(0, violator_name, 0, sd->status.account_id);

	return 0;
}

ACMD_FUNC(fateshield_unblock_account_id)
{
	int violator_aid;
	const char* command_info = "Wrong input (usage: @fateshield_unblock_account_id <account ID>)";
	...
	chrif_fateshield_req_unblock(0, NULL, violator_aid, sd->status.account_id);

	return 0;
}

ACMD_FUNC(fateshield_unblock_unique_id)
{
	unsigned int violator_unique_id;
	const char* command_info = "Wrong input (usage: @fateshield_unblock_unique_id <unique ID>)";
	...
	chrif_fateshield_req_unblock(violator_unique_id, NULL, 0, sd->status.account_id);
...
// (^~_~^) FateShield Start
		ACMD_DEF(fateshield_block_nick),
		ACMD_DEF(fateshield_block_account_id),
		ACMD_DEF(fateshield_block_unique_id),
		ACMD_DEF(fateshield_unblock_nick),
		ACMD_DEF(fateshield_unblock_account_id),
		ACMD_DEF(fateshield_unblock_unique_id),
// (^~_~^) FateShield End
