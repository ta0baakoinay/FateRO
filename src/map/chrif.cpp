...
// (^~_~^) FateShield Start
		if (cmd == FATESHIELD_C2M_BLOCK_ACK)
		{
			if (chrif_fateshield_ack_block(fd) == true)
				continue;
			else
				return 0;
		}
		else if (cmd == FATESHIELD_C2M_UNBLOCK_ACK)
		{
			if (chrif_fateshield_ack_unblock(fd) == true)
				continue;
			else
				return 0;
		}
...
// (^~_~^) FateShield Start
int chrif_fateshield_req_block(unsigned int unique_id, const char* violator_name, unsigned int violator_aid, const char* initiator_name, unsigned int initiator_aid, const char* unban_time_str, const char* reason_str)
{
	unsigned int offset;
	char send_buffer[2 + 4 + 4 + 4 + FATESHIELD_TIME_STR_LENGTH + FATESHIELD_REASON_LENGTH + NAME_LENGTH + NAME_LENGTH];

	chrif_check(-1);

	memset(send_buffer, '\0', sizeof(send_buffer));

	WBUFW(send_buffer, 0) = FATESHIELD_M2C_BLOCK_REQ;
	...
	if (unban_time_str != NULL)
		safestrncpy((char*)WBUFP(send_buffer, offset), unban_time_str, FATESHIELD_TIME_STR_LENGTH);
	offset += FATESHIELD_TIME_STR_LENGTH;

	if (reason_str != NULL)
		safestrncpy((char*)WBUFP(send_buffer, offset), reason_str, FATESHIELD_REASON_LENGTH);
...
bool chrif_fateshield_ack_block(int fd)
{
	map_session_data* sd;
	...
	unsigned int packet_len = (2 + 4 + 4 + 4 + FATESHIELD_TIME_STR_LENGTH + FATESHIELD_REASON_LENGTH + FATESHIELD_RESULT_STR_LENGTH);
	...
	safestrncpy(unban_time_str, (char*)RFIFOP(fd, offset), FATESHIELD_TIME_STR_LENGTH);
	offset += FATESHIELD_TIME_STR_LENGTH;

	safestrncpy(reason_str, (char*)RFIFOP(fd, offset), FATESHIELD_REASON_LENGTH);
	offset += FATESHIELD_REASON_LENGTH;

	safestrncpy(result_str, (char*)RFIFOP(fd, offset), FATESHIELD_RESULT_STR_LENGTH);
	...
	if (violator_aid != 0 && (sd = map_id2sd(violator_aid)) != NULL)
	{
		...
		for (sd = (TBL_PC*)mapit_first(iter); mapit_exists(iter); sd = (TBL_PC*)mapit_next(iter))
		{
			if (session[sd->fd]->fateshield_info.unique_id == unique_id)
			{
				fateshield_send_info(sd->fd, FATESHIELD_INFO_BANNED, message_info);
...
int chrif_fateshield_req_unblock(unsigned int unique_id, const char* violator_name, unsigned int violator_aid, unsigned int initiator_aid)
{
	unsigned int offset;
	...
	WBUFW(send_buffer, 0) = FATESHIELD_M2C_UNBLOCK_REQ;
	WBUFL(send_buffer, 2) = unique_id;
	WBUFL(send_buffer, 6) = violator_aid;
	WBUFL(send_buffer,10) = initiator_aid;
	offset = (2 + 4 + 4 + 4);
...
bool chrif_fateshield_ack_unblock(int fd)
{
	map_session_data* sd;
	int initiator_aid, offset;
	char result_str[FATESHIELD_RESULT_STR_LENGTH];
	unsigned int packet_len = (2 + 4 + FATESHIELD_RESULT_STR_LENGTH);
...
int chrif_fateshield_save_report(map_session_data* sd, const char* report_str)
{
	struct socket_data* s = session[sd->fd];

	unsigned int offset;
	char send_buffer[2 + 4 + 4 + 4 + FATESHIELD_REPORT_LENGTH + NAME_LENGTH];

	chrif_check(-1); //Character is saved on reconnect.

	memset(send_buffer, '\0', sizeof(send_buffer));

	WBUFW(send_buffer, 0) = FATESHIELD_M2C_SAVE_REPORT;
	WBUFL(send_buffer, 2) = s->fateshield_info.unique_id;
	...
	safestrncpy((char*)WBUFP(send_buffer, offset), report_str, FATESHIELD_REPORT_LENGTH);
	offset += FATESHIELD_REPORT_LENGTH;
