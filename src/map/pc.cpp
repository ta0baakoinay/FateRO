	...
	if (item->equip&EQP_AMMO) {
		...
		switch (item->subtype) {
			case AMMO_ARROW:
				...
				break;
			case AMMO_DAGGER:
				if (!pop_pc && !pc_checkskill(sd, AS_VENOMKNIFE))
					return ITEM_EQUIP_ACK_FAIL;
				break;
			case AMMO_BULLET:
			case AMMO_SHELL:
...
void pc_reg_received(map_session_data *sd)
{
	...
	// CRITICAL: Properly initialize deposit structure first
	sd->deposit.calc = false;
	sd->deposit.items.clear();
	sd->deposit.bonus.clear();
	sd->deposit.bonus.reserve(100); // Pre-allocate space to prevent reallocation issues
...
enum e_additem_result pc_additem(map_session_data *sd,struct item *item,int32 amount,e_log_pick_type log_type, bool favorite) {
	...
	// Fake Players (population engine shells) never accumulate carry weight, so
	// they can loot indefinitely while farming. Real players are unaffected.
...
int32 pc_checkbaselevelup(map_session_data *sd) {
	...
	// Fake players (population engine shells) can never reach Level 99: hold them
	// at 98 and drop the surplus exp. Real players are unaffected.
	const bool pop_pc = population_engine_is_population_pc(sd->id);
	if (pop_pc && sd->status.base_level >= 98) {
	...
	do {
		...
		// Fake players stop at 98 even with multi_level_up enabled.
		if( pop_pc && sd->status.base_level >= 98 ){
			sd->status.base_exp = 0;
			break;
		}
